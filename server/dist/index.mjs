...
function connectToDatabase() {
  mongoose.connect(MONGO_URL).then(() => console.log("Connected to MongoDB")).catch((error) => console.error("Error connecting to MongoDB:", error));
}
    ...
    return __async(this, null, function* () {
      ...
      try {
        ...
        if (userData.length > 0) {
          const userVerify = yield bcrypt.compare(password, userData[0].password);
          if (userVerify) {
    ...
    return __async(this, null, function* () {
      try {
        ...
        if (userDetails && (userDetails == null ? void 0 : userDetails.length) > 0) {
          ...
          let encrptpassword = yield bcrypt.hash(password, 10);
...
var fileSchema = new Schema2({
  userId: { type: Schema2.Types.ObjectId, ref: "user" },
  filename: String,
  uniqueCode: String
});
var fileModel = mongoose3.model("File", fileSchema);
